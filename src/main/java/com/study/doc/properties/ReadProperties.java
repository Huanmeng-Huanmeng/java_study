package com.study.doc.properties;
...
public class ReadProperties {
    ...
    public static final String ERROR_MESSAGE_OF_FARMCONTROL_SECOND_CHECK = "不符合能管的关键配置文件校验的第二项：【agc avc】";
    ...
    public static final String ERROR_MESSAGE_OF_FARMCONTROL_FOURTH_CHECK = "不符合能管的关键配置文件校验的第四项：【一次调频的配置】";
    ...
    public static String FILE_NAME_PREFIX = "C:/Users/root/Desktop/BLFC/scada/software/farmcontrol/";
    public static String APPLICATION_FILE_NAME = "application.properties";
    public static String FARMCONFIG_FILE_NAME = "sany/settings/farmconfig.properties";
    public static String CHANNEL_FILE_NAME = "sany/settings/channel.xml";
    public static String IEC_PUBS_FILE_NAME = "sany/settings/iec-pubs.xml";
    public static String IEC_TAGS_FILE_NAME = "sany/settings/iec-tags.xml";
    public static String MODBUS_PUBS_FILE_NAME = "sany/settings/modbus-pubs.xml";
    public static String MODBUS_TAGS_FILE_NAME = "sany/settings/modbus-tags.xml";
    ...
    public static void main(String[] args) {
        farmControlConfigCheck();
    }

    public static void farmControlConfigCheck() {
        ...
        for (String example: list) {
            ...
            String applicationFileName = filePrefix + APPLICATION_FILE_NAME;
            String farmconfigFileName = filePrefix + FARMCONFIG_FILE_NAME;
            String channelFileName = filePrefix + CHANNEL_FILE_NAME;
            String iecPubsFileName = filePrefix + IEC_PUBS_FILE_NAME;
            String iecTagsFileName = filePrefix + IEC_TAGS_FILE_NAME;
            String modbusPubsFileName = filePrefix + MODBUS_PUBS_FILE_NAME;
            String modbusTagsFileName = filePrefix + MODBUS_TAGS_FILE_NAME;
            log.info("-----" + example + "风场校验开始-----");
            test6(applicationFileName, farmconfigFileName, channelFileName,
                    iecPubsFileName, iecTagsFileName, modbusPubsFileName, modbusTagsFileName);
    ...
    public static void test6(String applicationFileName, String farmconfigFileName, String channelFileName, String iecPubsFileName, String iecTagsFileName, String modbusPubsFileName, String modbusTagsFileName) {
        ...
        if (farmConfigPropertiesCheck) {
            ...
            if (!firstCheck) {
                ...
                errorMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_FIRST_CHECK + "\n");
            }
        } else {
            log.error(ERROR_MESSAGE_OF_FARMCONTROL_FIRST_CHECK);
            errorMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_FIRST_CHECK + "\n");
        ...
        if (applicationPropertiesCheck) {
            ...
            if (!secondCheck) {
                ...
                errorMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_SECOND_CHECK + "\n");
            }
        } else {
            log.error(ERROR_MESSAGE_OF_FARMCONTROL_SECOND_CHECK);
            errorMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_SECOND_CHECK + "\n");
        ...
        // 3.风机采集点  sany/settings/modbus-tags.xml 报错级别
        ...
        if (farmConfigPropertiesCheck) {
            ...
            if (!thirdCheck) {
                ...
                errorMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_THIRD_CHECK + "\n");
            }
        } else {
            log.error(ERROR_MESSAGE_OF_FARMCONTROL_THIRD_CHECK);
            errorMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_THIRD_CHECK + "\n");
        ...
        if (applicationPropertiesCheck) {
            fourthCheck = fourthCheck(applicationProperties, channelDc, modbusPubsFileName, iecPubsFileName);
            if (!fourthCheck) {
                log.error(ERROR_MESSAGE_OF_FARMCONTROL_FOURTH_CHECK);
                warningMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_FOURTH_CHECK + "\n");
            }
        } else {
            log.error(ERROR_MESSAGE_OF_FARMCONTROL_FOURTH_CHECK);
            warningMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_FOURTH_CHECK + "\n");
        ...
        if (applicationPropertiesCheck) {
            ...
            if (!fifthCheck) {
                ...
                warningMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_FIFTH_CHECK);
            }
        } else {
            log.error(ERROR_MESSAGE_OF_FARMCONTROL_FIFTH_CHECK);
            warningMessage.append(ERROR_MESSAGE_OF_FARMCONTROL_FIFTH_CHECK);
    ...
    public static boolean firstCheck(Properties farmConfigProperties) {
    ...
    // 能管的关键配置文件校验：2.agc avc
    public static boolean secondCheck(Properties applicationProperties, Document channelDc, String modbusPubsFileName, String iecPubsFileName) {
        ...
        if (agcProperty == null || avcProperty == null) {
            log.error("两个属性【[{}]，[{}]】都不能为空。", GRIDCOMMANDLISTENER_AGC, GRIDCOMMANDLISTENER_AVC);
            return false;
        }
        ...
        if (agcPropertySplit.length != 4 || avcPropertySplit.length != 4) {
            log.error("两个属性【[{}]，[{}]】按冒号（:）截取出来的字符串数组长度不全为4。", GRIDCOMMANDLISTENER_AGC, GRIDCOMMANDLISTENER_AVC);
            return false;
        }

        if (!EMS.equals(agcPropertySplit[2]) || !EMS.equals(avcPropertySplit[2])) {
            log.error("两个属性【[{}]，[{}]】截取出来的字符串数组的第三项必须全为ems。", GRIDCOMMANDLISTENER_AGC, GRIDCOMMANDLISTENER_AVC);
            return false;
        }
            ...
            if (channelRootElement != null) {
                ...
                if (channels2Element != null) {
                    ...
                    for (Element example: channel2Elements) {
                        ...
                        if (agcPropertyBoolean && avcPropertyBoolean) {
                            break;
                        }
                    }
                    if (!agcPropertyBoolean || !avcPropertyBoolean) {
    ...
    public static boolean thirdCheck(Properties farmConfigProperties, Document channelDc, Document modbusTagsDc) {
    ...
    // 能管的关键配置文件校验：4.一次调频的配置
    public static boolean fourthCheck(Properties applicationProperties, Document channelDc, String modbusPubsFileName, String iecPubsFileName) {
        String primaryProperty = applicationProperties.getProperty(GRIDCOMMANDLISTENER_PRIMARY);
        if (primaryProperty == null) {
            log.error("属性【{}】不能为空。", GRIDCOMMANDLISTENER_PRIMARY);
            return false;
        }
        String[] primaryPropertySplit = primaryProperty.trim().split(COLON);
        if (primaryPropertySplit.length != 4) {
            log.error("属性【{}】按冒号（:）截取出来的字符串数组长度不为4。", GRIDCOMMANDLISTENER_PRIMARY);
            return false;
        }
        if (!EMS.equals(primaryPropertySplit[2])) {
            log.error("属性【{}】截取出来的字符串数组的第三项必须为ems。", GRIDCOMMANDLISTENER_PRIMARY);
            return false;
        }
    ...
    // 能管的关键配置文件校验：5.并网点
    public static boolean fifthCheck(Properties applicationProperties, Document channelDc, String iecTagsFileName, Document modbusTagsDc) {
    ...
    public static boolean farmControlConfigCheck2(Element example, String[] propertySplit, String modbusFileName, String iecFileName){
    ...
    public static boolean readPropertiesFromFile(Properties properties, String fileName){
    ...
    public static Document readDocumentFromFile(String fileName) {
        ...
        try {
            ...
            log.error("文件解析报错DocumentException", fileName);
        }
        if (document == null) {
            log.error("文件解析报错DocumentException", fileName);
        }
